	...
	<head>
		...
		<script type="text/javascript">
			...
			{			
				...
				// get suitable node (the span-tag)
				var targetnode;
				<![CDATA[
				if ( ieVer > -1 && ieVer  > 8.0 ) 
				{
					targetnode = node.firstChild.nextSibling.firstElementChild.firstElementChild.firstElementChild.firstElementChild.firstChild;
				}
				else
				{
					targetnode = node.firstChild.firstChild.firstChild.firstChild.firstChild.firstChild; 
				}
				]]>
				
				SwitchTreeSymbol(targetnode);
			}

			function SwitchTreeSymbol(node) // function which switches the + into - and vice versa
			{			
				if ( node != null) // check if node is available
				{
					s = new String(node.nodeValue); // get the node's text
					if ((i = s.indexOf("+")) != -1) // switch tree symbol into it's opposite
					{
						node.nodeValue = (s.substr(0,i) + "-" + s.substr(i+1,s.length));
					}
...
<xsl:template name="SingleScan"> <!-- single scan drive and folder details -->
	...
	<xsl:apply-templates select="TPatternList" />	
	<h3>
...
<xsl:template match="TPatternList">
	<xsl:if test="Pattern">
		<xsl:text>Filter: </xsl:text>
		<xsl:for-each select="Pattern">
			"<xsl:value-of select="@Text" />"
			<xsl:if test="@PatternType = 1"> (Exclude) </xsl:if>
			<xsl:if test="@PatternType = 0"> (Include) </xsl:if>			
			<xsl:if test="position() != last()">, </xsl:if>
		</xsl:for-each>
	</xsl:if>
</xsl:template>
